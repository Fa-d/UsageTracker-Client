package com.example.screentimetracker.integration
...
class TimeRestrictionsWeeklyInsightsIntegrationTest {
    ...
    private val testTimeRestriction = TimeRestriction(
        ...
        startTimeMillis = TimeUnit.HOURS.toMillis(9), // 9 AM
        endTimeMillis = TimeUnit.HOURS.toMillis(17), // 5 PM
    ...
    fun setup() {
        ...
        coEvery { repository.insertTimeRestriction(any()) } returns Unit
        coEvery { repository.updateTimeRestriction(any()) } returns Unit
        coEvery { repository.deleteTimeRestriction(any()) } returns Unit
        coEvery { repository.incrementTimeRestrictionViolation(any()) } returns Unit
        
        coEvery { repository.getAggregatedSessionDataForDayFlow(any(), any()) } returns flowOf(testSessionData)
        coEvery { repository.getDailyAppSummaries(any(), any()) } returns flowOf(testDailyAppSummaries)
    ...
    fun `time restrictions should affect weekly insights data`() = runTest {
        ...
        assertNotNull("Instagram should appear in top apps", instagramUsage)
        ...
        assertTrue("Weekly report should have meaningful insights", weeklyReport.insights.isNotEmpty())
        
        // Total screen time should be sum of all app usage
        val expectedTotal = testSessionData.sumOf { it.totalDuration }
        assertEquals(expectedTotal, weeklyReport.totalScreenTimeMillis)
    }

    @Test
    fun `time restriction violations should be tracked in weekly insights`() = runTest {
        // Given - Add violation to restriction
        val violatedRestriction = testTimeRestriction.copy(violationCount = 5)
        coEvery { repository.getAllTimeRestrictions() } returns flowOf(listOf(violatedRestriction))
        
        // When
        val restrictions = timeRestrictionUseCase.getAllTimeRestrictions().value
        val weeklyReport = weeklyInsightsUseCase.generateWeeklyReport()
        
        // Then
        val restriction = restrictions.first()
        assertEquals(5, restriction.violationCount)
        
        // Weekly report should still show data even with violations
        assertTrue("Weekly report should have data despite violations", weeklyReport.totalScreenTimeMillis > 0)
        assertTrue("Should have insights about restriction effectiveness", weeklyReport.insights.isNotEmpty())
    }
    ...
    fun `productivity insights should consider time restrictions`() = runTest {
        ...
        assertTrue("Should have productivity data for all hours", restrictionHours.isNotEmpty())
        assertTrue("Should have data for non-restriction hours", nonRestrictionHours.isNotEmpty())
        
        // Verify productivity scores are calculated
        restrictionHours.forEach { hour ->
            assertTrue("Productivity score should be valid", hour.productivity >= 0f && hour.productivity <= 1f)
        }
    }
    ...
    fun `category insights should reflect restricted app usage`() = runTest {
        ...
        assertNotNull("Should have Social category", socialCategory)
        
        // Find entertainment category (Spotify)
        val entertainmentCategory = categoryInsights.find { it.categoryName == "Entertainment" }
        assertNotNull("Should have Entertainment category", entertainmentCategory)
        
        // Verify categories have usage data
        if (socialCategory != null) {
            assertTrue("Social category should have usage time", socialCategory.totalTimeMillis > 0)
            assertTrue("Social category should have percentage", socialCategory.percentageOfTotal > 0)
        }
    }
    ...
    fun `time restrictions should not break weekly report generation`() = runTest {
        ...
        val multipleRestrictions = listOf(
            ...
            testTimeRestriction.copy(
                id = 2,
                startTimeMillis = TimeUnit.HOURS.toMillis(20), // 8 PM
                endTimeMillis = TimeUnit.HOURS.toMillis(23), // 11 PM
                daysOfWeek = setOf(6, 7) // Weekend
            )
        )
        ...
        assertNotEquals(WeeklyInsightsUseCase.WeeklyReport.empty(), weeklyReport)
        assertTrue("Should have usage data", weeklyReport.totalScreenTimeMillis > 0)
        assertFalse("Should have top apps", weeklyReport.topApps.isEmpty())
    }
    ...
    fun `time restriction status should be available during weekly report generation`() = runTest {
        ...
        assertTrue("Should be able to check restriction status", isRestricted != null)
        assertNotNull("Weekly report should generate successfully", weeklyReport)
    }
    ...
    fun `weekly notification should work with time restrictions active`() = runTest {
        ...
        val restrictions = timeRestrictionUseCase.getAllTimeRestrictions().value
        assertTrue("Time restrictions should still be available", restrictions.isNotEmpty())
    }
    ...
    fun `error in time restrictions should not break weekly insights`() = runTest {
        ...
        assertNotNull("Weekly report should still be generated", weeklyReport)
        assertTrue("Should have usage data despite restriction error", weeklyReport.totalScreenTimeMillis > 0)
    }
    ...
    fun `time restriction data should be consistent across both use cases`() = runTest {
        ...
        // Then
        val restrictedApps = restrictions.filter { it.isActive }.map { it.packageName }
        ...
        if (restrictedInReport.isNotEmpty()) {
            restrictedInReport.forEach { packageName ->
                ...
                assertNotNull("Restricted app should have usage data", appUsage)
                assertTrue("Restricted app should have some usage", appUsage!!.totalTimeMillis > 0)
            }
        }
    }
}